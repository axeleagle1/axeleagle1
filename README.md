...
|----------|-------|------|----------|
| **Polymarket NegRisk** | Polygon (EVM) | Prediction Market | 1 Critical (Cantina Bounty) |
...
| **CollectorRoll** | Solana | Pokemon Gacha Platform | 1 Critical |
...
|----------|---------------|--------|
| **Polymarket NegRisk** | Non-deterministic sibling resolution | When multiple sibling questions satisfy TRUE, outcome depends on `resolveQuestion()` execution order, not oracle state. Submitted via **Cantina bounty program**. |
...
| **CollectorRoll** | Memecoin reward loop drained shared inventory pool | Memecoin smart contract's automated buy/reward engine depleted common packs on the Pokemon gacha platform, breaking normal openings and forcing platform offline. Design flaw persists. |
...
- [Audit Findings](https://github.com/axeleagle1/audit-findings)
